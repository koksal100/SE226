...
while True:
    password=""
    user_number = int(input("give a value between 3 and 7 inclusive"))
...
while True:
    user_number=int(input("Please give a number between 1 and 30 both inclusive:"))
    if user_number<1 or user_number>30:
        continue

    dict.pop(user_number)
    break
...
print("Name"+" "*(8-len("name"))+ "Age")


for key in united_we_stand:
    print(key+(" "*(8-len(key))),united_we_stand[key])

del united_we_stand["Nat"]
print(united_we_stand)
print(sorted(united_we_stand.items()))
print(sorted(united_we_stand.items(),key = lambda x:x[1]))
max=max(united_we_stand.items(),key= lambda x:x[1])
print(max)
min=min(united_we_stand.items(),key= lambda x:x[1])